...
private:
	static constexpr IntType fnv(IntType hash, char const* pos) {
		return *pos == '\0'
		  ? hash
...
std::ostream& operator<<(std::ostream& out, printable_typename<T>) {
	char const* implementation_typename = typeid(T).name();
#ifdef __GNUC__
	int status;
	char const* pretty_name = abi::__cxa_demangle(implementation_typename, NULL, NULL, &status);
	if (status == 0)
		out << pretty_name;
