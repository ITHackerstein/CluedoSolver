...
void Solver::learn_from_suggestion(Suggestion const& suggestion, bool infer_new_info) {
	...
	for (auto player_index = increment_cycling_index(suggestion.suggesting_player_index);; player_index = increment_cycling_index(player_index)) {
		if (player_index == suggestion.suggesting_player_index) {
			break;
		} else if (player_index == suggestion.responding_player_index) {
			if (suggestion.response_card) {
				learn_player_card_state(*suggestion.responding_player_index, *suggestion.response_card, true, false);
			} else {
				learn_player_has_any_of_cards(*suggestion.responding_player_index, { suggestion.suspect, suggestion.weapon, suggestion.room }, false);
			}
			break;
		}
