...
public:
	...
	public:
		...
		constexpr void advance_to_next_index() {
			while (m_index < m_set_ref.size() && !m_set_ref.test(m_index))
				++m_index;
		}
	...
	constexpr bool contains(Card card) const { return m_set.test(static_cast<std::size_t>(card)); }

	constexpr bool insert(Card card) {
		if (contains(card))
			return true;

		m_set.set(static_cast<std::size_t>(card));
		return false;
	}

	constexpr void erase(Card card) { m_set.reset(static_cast<std::size_t>(card)); }
